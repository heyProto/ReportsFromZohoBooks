...
args
  .option('token', {
    alias: 't',
    describe: 'Access token to be used for pulling data'
  })
  .option('orgid', {
    alias: 'o',
    describe: 'Organization ID for Zoho Books account'
  })
  .option('projectid', {
    alias: 'p',
    describe: 'Project ID for generating report'
  })
  .option('attachment', {
    alias: 'a',
    describe: 'Choose to download attachments for report',
    choices: ['y', 'n']
  })
  .option('exchangerate', {
    alias: 'e',
    describe: 'USD rate to be used (uncalculated by default)'
  })
  .demandOption(['token', 'orgid', 'projectid', 'attachment'])
...
scheduleRequests(axiosInstance, 1000)
...
axiosInstance
  ...
  .catch(err => {
    ...
    console.error('Error while retrieving project: ', err.response.data)
...
function findDetails () {
  axios
    ...
    .catch(err => {
      console.error(
        'Error will retrieving project details: ',
        err.response.data
      )
    })
}
...
function scheduleRequests (axiosInstance, intervalMs) {
  ...
  const scheduler = config => {
    console.log('Starting request to URL: "', config.url, '"')
    apiCallCount++
    const now = Date.now()
